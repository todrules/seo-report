angular.module('app.services', [])
	   ...
	   .factory('ErrorService', ['$mdDialog', function($mdDialog) {
		   var error = {};
		   var showConfirm = function() {
			   $mdDialog.show($mdDialog.alert()
									   ...
									   .textContent('That was unexpected! Refresh and try again.')
									   .ariaLabel('Error')
									   .ok('Let\'s Go'));
		   };
	   ...
	   .factory('DateService', function() {
		   ...
		   dates.getStartEndDates = function(myDate) {
			   var d = new Date(myDate);
			   var month = d.getMonth() + 1;
			   var year = d.getFullYear();
			   var next = new Date(year, month, 1);
			   var end = new Date(next - 1);
			   ...
			   startDate = year + filler + month + '-01';
			   endDate = year + filler + month + filler + lastday;
	   ...
	   .service('StatService', ['GApi', 'DateService', '$q', function(GApi, DateService, $q) {
		   ...
		   var dates = [];
		   var params = {
			   startDate: null, endDate: null, dimensions: ["date"]
		   };
		   var url = '';
		   var dataObj = {
			   clicks: null,
		   ...
		   function getChart(metric) {
			   var statChartObj = [{type: 'date', displayName: 'Month', name: 'month', field: 'month'}, {
				   type: 'number', displayName: 'Id', name: 'companyId', field: 'companyId'
			   ...
			   }, {type: 'number', displayName: 'Sessions', name: 'sessions', field: 'sessions'}, {
				   type: 'number', displayName: 'Bounces', name: 'bounces', field: 'bounces'
			   }, {
				   type: 'number', displayName: 'Bounce Rate', name: 'bouncerate', field: 'bounceRate'
			   }, {type: 'number', displayName: 'Duration', name: 'duration', field: 'duration'}, {
				   type: 'number', displayName: 'Page Views', name: 'pageviews', field: 'pageViews'
			   }, {type: 'number', displayName: 'Pages / Session', name: 'ppv', field: 'ppv'}, {
				   type: 'number', displayName: 'Unique Views', name: 'uniqueviews', field: 'uniquePageViews'
			   }, {type: 'number', displayName: 'Page Load', name: 'pageload', field: 'pageLoadTime'}];
			   return statChartObj;
		   }
		   ...
		   var landingPageObj = [{
			   type: 'string', displayName: 'Landing Page', name: 'landingPage', field: 'landingPage'
		   }, {
			   type: 'number', displayName: 'Clicks', name: 'clicks', field: 'clicks'
		   }, {
			   type: 'number', displayName: 'Impressions', name: 'impressions', field: 'impressions'
		   ...
		   var metricObj = {
			   clicks: {
				   value: null, displayName: 'Clicks', name: 'clicks', analyticsTag: null
			   }, impressions: {
				   value: null, displayName: 'Impressions', name: 'impressions', analyticsTag: null
			   }, ctr: {
				   value: null, displayName: 'CTR', name: 'ctr', analyticsTag: null
			   }, position: {
				   value: null, displayName: 'Position', name: 'position', analyticsTag: null
			   }, sessions: {
				   value: null, displayName: 'Sessions', name: 'sessions', analyticsTag: 'ga:sessions'
			   }, bounces: {
				   value: null, displayName: 'Bounces', name: 'bounces', analyticsTag: 'ga:bounces'
			   }, bouncerate: {
				   value: null, displayName: 'Bounce Rate', name: 'bouncerate', analyticsTag: 'ga:bounceRate'
			   }, duration: {
				   value: null, displayName: 'Duration', name: 'duration', analyticsTag: 'ga:avgSessionDuration'
			   }, pageviews: {
				   value: null, displayName: 'Page Views', name: 'pageviews', analyticsTag: 'ga:pageviews'
			   }, ppv: {
				   value: null, displayName: 'Pages / Session', name: 'ppv', analyticsTag: 'ga:pageviewsPerSession'
			   }, uniqueviews: {
				   value: null, displayName: 'Unique Views', name: 'uniqueviews', analyticsTag: 'ga:uniquePageviews'
			   }, pageload: {
				   value: null, displayName: 'Page Load', name: 'pageload', analyticsTag: 'ga:avgPageLoadTime'
			   }
		   };
	
		   function getSearchAnalytics(url, myDate) {
			   var d = new Date(myDate);
		
			   var deferred = $q.defer();
			   dates = DateService.getStartEndDates(d);
			   params = {
		   ...
		   function getGoogleAnalytics(id, myDate) {
			   var d = new Date(myDate);
			   var deferred = $q.defer();
			   var id = id;
			   console.log(id);
		   ...
		   return {
			   getWebmasterData: function(url, myDate) {
				   var myDate = myDate;
				   var deferred = $q.defer();
				   getSearchAnalytics(url, myDate)
				   ...
				   return deferred.promise;
			   }, getAnalyticsData: function(id, myDate) {
				   var myDate = myDate;
				   var id = id;
				   console.log(id);
				   var deferred = $q.defer();
				   getGoogleAnalytics(id, myDate)
				   ...
				   var obj = dataObj;
				   return obj;
			   }, getChartObj: function(metric) {
				   return getChart(metric);
