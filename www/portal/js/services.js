angular.module('app.services', [])
	...
	.factory('CompanyService',  [ 'Company', 'StorageService',function(Company, SS) {
		...
		company.getAll = function() {
			...
			if(checked == false) {
				Company.find({filter:{include: [{keywords: 'keywordMetrics'},{competitors: 'competitorMetrics'},'metrics']}})
		...
		company.getCompanyById = function(id) {
			if(id === undefined) {
				...
				if(job == false) {
					Company.findById({id: id},{include: [{keywords: 'keywordMetrics'},{competitors: 'competitorMetrics'},'metrics']}).$promise.then(function(res) {
						var handler = res;
						if(handler.length > 0) {
	...
	.factory('DashConfig', function () {
		...
		dashconfig.getKeyGridConfig = function() {
			var reportMetrics = [
				...
				{name: 'minVolume', headerCellClass: 'plain', headerTooltip: 'How often a word or' +
				' phrase is' +
				' searched for on' +
				' Google each month.'},
